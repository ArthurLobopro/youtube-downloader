...
export function DownloadPage() {
  const {
    state: { info, url },
  } = useLocation() as DownloadPageState;
  ...
  async function onDownload() {
    ...
    const result = await window.api.validateAndDownload(
      url,
      savePath,
      saveFormat,
    );
  ...
  return (
    <div className="flex flex-col gap-1 w-full items-center relative">
      <Link
        to="/"
        className="fixed top-2 left-2 rounded-full hover:bg-accent p-2"
      >
      ...
      {info && (
        <iframe
          ...
          src={`https://www.youtube-nocookie.com/embed/${info.videoDetails.videoId}`}
          frameBorder="0"
        ...
        {info && (
          <>
            <label className="flex w-max gap-1">
              Salvar como:
              <select
                className="border-solid border-1 border-white rounded"
                value={saveFormat}
                onChange={(ev) =>
                  setSaveFormat(ev.currentTarget.value as saveFormat)
                }
              >
            ...
            <div className="flex gap-1 items-center">
              ...
              <Button onClick={selectFolder} className="h-full">
                Escolher
              </Button>
            </div>
